...
@required
-(void)peripheralUpdateValue:(NSData *_Nullable)data;
 ...
 *  @param failed  失败回调
 */
- (void) startScanningPeripheralWithDeviceType:(XYDeviceType)type timeout:(float) timeout complete:(ScanDeviceCompletionHandle)complete failed:(ScanDeviceFailedBlock _Nonnull )failed;
...
/**
 获取系统已经配对的设备
 */
-(NSArray *_Nullable)getSystemLinkPeripheral;

@end

@interface ScanDevice : NSObject

@property (nonatomic,strong,readonly) CBPeripheral * _Nullable peripheral;
